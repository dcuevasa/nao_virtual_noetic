...
class NaoCmdVelFakeDriver:
    def __init__(self):
        rospy.init_node('nao_cmd_vel_fake_driver')
        
        # Parámetros
        self.model_name = rospy.get_param('~model_name', 'nao_v40')  # Nombre del modelo en Gazebo
        self.update_rate = rospy.get_param('~update_rate', 50.0)  # Hz
        self.max_linear_speed = rospy.get_param('~max_linear_speed', 0.2)  # m/s
        self.max_angular_speed = rospy.get_param('~max_angular_speed', 0.2)  # rad/s
        ...
        try:
            ...
            if initial_state.success:
                ...
                self.current_x = 0.0
                self.current_y = 0.0
                self.current_z = 0.334  # Altura predeterminada de NAO
                self.current_yaw = 0.0
        except rospy.ServiceException as e:
            rospy.logerr(f"Llamada a servicio fallida: {e}")
            self.current_x = 0.0
            self.current_y = 0.0
            self.current_z = 0.334
            self.current_yaw = 0.0
        ...
        rospy.loginfo("Controlador fake de cmd_vel para NAO inicializado")
    ...
    def cmd_vel_callback(self, msg):
        ...
        limited_msg.linear.z = 0.0  # NAO no puede volar
